...
import { MY_CLUB, SLOT_COMPAT, WEEK_MS, PLAYER_ROLES, PLAN_B_SCENARIOS } from '../constants.js'
...
After the 5 sub lines, add a short "Plan B" block: 1-3 lines (not all 6 scenarios — only the ones genuinely worth flagging for this specific matchup), each formatted "Scenario name → reaction direction", using only the scenario names from the Plan B list in the game mechanics reference further below. The "reaction direction" is a plain-language tactical description ONLY (e.g. "push men forward and get more direct", "shut things down and see the game out") — never a specific named Plan or an exact Mentality/Style/etc. value, since the exact scenario→setting mapping isn't confirmed yet. If nothing about this particular matchup makes any scenario stand out, it's fine to give one line with a sensible generic default (e.g. "Losing by 2+ → push for an equalizer") rather than omitting the block outright — but never invent a scenario name that isn't in the reference list, and remember Plan B fires at most once per match with no personnel changes (that's what Section 4's own subs are for).
...
- Plan B: up to 6 scenario-triggered whole-team plan shifts, each firing at MOST ONCE per match and involving NO personnel changes (personnel changes are what the 5 subs above are for) — the fixed scenarios are: ${planBRef}. The exact mapping from a scenario to a specific Mentality/Style/etc. value isn't confirmed yet, so only describe a plausible tactical DIRECTION for a scenario (e.g. "push for goals", "shut up shop and protect the lead") — never assert a specific named Plan or exact instruction value for a Plan B reaction. Precedence, per the game's own on-page copy: a later sub's Instruction overrides an earlier Plan B, and vice versa — whichever one fires most recently wins and sets the team's approach from that point on.`);
...
- Section 4 ends with a short Plan B block (1-3 lines, scenario names only from the fixed list in the game mechanics reference) — a reaction DIRECTION only, never a specific named Plan or exact instruction value.
