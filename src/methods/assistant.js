...
Defensive corner — Press: [Hold Shape/Press Taker]`);
    ...
    if (squad.length) {
      ...
      const rankBy = key => squad.slice().filter(p => p[key] != null).sort((a, b) => b[key] - a[key]).slice(0, 10)
        .map(p => `${p.Player} ${p[key]}`).join(', ');
      lines.push(`\nSquad set-piece rankings (pre-sorted so you don't have to compute this yourself — pick whichever of these names is in your recommended XI, in the order shown, for the taker and the "next best" comparison; do NOT substitute a different number for a name than the one shown here, and do NOT mix numbers between the three lists):
Free kicks, highest to lowest (top 10): ${rankBy('Free kicks')}
Penalties, highest to lowest (top 10): ${rankBy('Penalties')}
Corners, highest to lowest (top 10): ${rankBy('Corners')}`);
    }
