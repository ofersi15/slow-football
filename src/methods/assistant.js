...
- Section 4 ends with a short Plan B block (2-4 scenarios from the fixed list, more if the opponent's own Plan B column shows them well-covered) — each line names a SPECIFIC named Plan from the fixed 7-option vocabulary, never a vague description or an invented name.
- Section 1 opens with a standalone scouting read on the opponent (their own strengths/weaknesses/threat profile) BEFORE tying it into our specific matchup — not straight into personnel matchups with no general picture first.
...
- ALL SIX of Section 3's lines have their own real reason, including "Balanced"/"Mixed"/"Medium" choices — not just the extreme-sounding ones. Sections 3 and 4 each carry a short specific reason, not a generic one that could apply to any opponent.
- Every one of Section 4's 5 lines has all 5 parts: literal window, condition in parens, both player names with "IN for", the Plan in parens, and a reason — none dropped, none merged into a summary.
...
- Section 1 named a specific personnel matchup (a player vs their direct opponent, or a combo like RB+RW vs their LB+LW), not just abstract team-level strengths/weaknesses, and its staleness read used the opponent-tactics table's computed GW status rather than eyeballing a raw GW number. It also read the opponent's actual per-player Roles (not generic mentality-based assumptions) and, if their Plan B is heavily configured, said whether that scripted reactivity itself hands us an opening.
