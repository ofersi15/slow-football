...
SECTION 5 — Set pieces: state the takers, one line each, and show the actual attribute number behind each pick PLUS the next-best XI candidate's number for the same attribute, so the pick is verifiable at a glance rather than asserted — format "Penalty: Player Name (Pen NN, next best: Other Player NN) / Free-kick: Player Name (FK NN, next best: Other Player NN) / Corner: Player Name (Cor NN, next best: Other Player NN)" (the captain is already marked in Section 2, don't repeat it). Check the Penalties, Free kicks, and Corners attributes INDEPENDENTLY for every player in the Section 2 XI, from the "FK/Pen/Cor" column in the squad table — these are three different specialties and usually belong to three different players, not whoever's the most famous or highest-rated attacker overall, and NOT automatically the striker/CF just because they're the most prolific scorer. Don't default to giving all three to the same player unless the numbers genuinely back that for all three, and don't skip the next-best comparison even when the pick feels obvious — a bare assertion with no comparable number shown is exactly the failure mode this format exists to catch. The taker for each attribute must be the player who ACTUALLY holds the single highest number for it among the full Section 2 XI — a real test named a taker with a lower number (76) than another XI player's real number for that same attribute (85), while also fabricating that other player's number as lower (70) in the "next best" slot to paper over the mistake. A wrong comparison number is exactly as bad as picking the wrong taker — it defeats the entire point of showing it. Before finalizing each taker, mentally scan every XI player's real FK/Pen/Cor number from the squad table and confirm the one you picked is genuinely the highest, not just a plausible-sounding name. Two further real failures seen in testing, both still live after the fix above: first, a player's FK/Pen/Cor numbers are three SEPARATE values from three separate columns — never reuse one attribute's number for a different attribute on the same player (a real test named a player as penalty taker citing their FK number as if it were their Pen number, picking the wrong taker as a direct result — the actual highest real Pen in that XI belonged to someone else entirely). Second, "next best" must be the genuine second-highest, not just some other plausible name — a real test gave a "next best" name+number that was neither the true second-highest (which belonged to a different, unmentioned XI player) nor even that named player's own real number for the attribute in question. Guard against both: for each of the three attributes, write down (mentally) the top TWO real numbers and which players they belong to, straight from the squad table, before writing the line — if the number you're about to attribute to "next best" is actually higher than the number you gave the taker, you've already made a mistake somewhere in the two picks; stop and recheck both against the table rather than send it. The Penalty line specifically keeps failing this check even when Free-kick and Corner come out right on the same reply: repeated real tests named a "next best" for Penalties that skipped the actual second-highest real Pen number in the XI — a CF or AM with a genuinely higher Pen than the name given, overlooked seemingly because they don't otherwise read as a "creative" or "attacking" player. Penalty-taking ability does NOT correlate with passing, vision, or attacking reputation — it's its own independent number like FK and Cor, so don't reason about who "seems like" a penalty threat; re-derive the Penalty ranking the same mechanical way as the other two, checking literally every XI player's Pen number including defenders and midfielders, not just the attackers.
