...
Role selection must be justified by the player's actual attributes, not just legal for their base position — being on the Player Roles list for a position makes a Role a legal choice, not a good one for THIS player. For wide/attacking Roles that lean on real pace or dribbling to do their job (Overlapper at FB; Traditional Winger, Wide Playmaker at WF), check "Spd" (Speed) and Dribbling in the squad table before assigning them — a player whose standout attributes are Shooting/Heading/Strength with modest Speed is a poor fit for a touchline-sprinting or overlap-heavy Role even though it's legal for their position; prefer Inside Forward (which relies on cutting inside and finishing, not out-and-out pace on the line) or a more restrained Role for that player instead, and if you deliberately keep them in a pace-reliant Role anyway, say why (e.g. their end product outweighs the mismatch). Also check Role COMBINATIONS on the same flank, not just each Role in isolation: an Overlapper full-back and a Traditional Winger/Wide Playmaker in front of them both want the same touchline space and crowd each other out — if you want a genuinely overlapping full-back, pair them with an Inside Forward ahead of them (which tucks infield, vacating the touchline for the overlap) rather than another wide-running Role; if you'd rather keep a touchline-hugging winger, pair them with a more conservative full-back Role (Two-Way Full Back / Defensive Full Back) behind them, not another Overlapper. State explicitly when two Roles on the same side are chosen to complement rather than duplicate each other.
...
- Plan B: up to 6 scenario-triggered whole-team plan shifts, each firing at MOST ONCE per match and involving NO personnel changes (personnel changes are what the 5 subs above are for) — the fixed scenarios are: ${planBRef}. Each scenario a manager configures gets assigned ONE named Plan from a fixed, real vocabulary confirmed straight off a real live opponent submission — ${planBNamedRef} — so recommend a SPECIFIC named Plan per scenario you address (e.g. "Losing by 2+ → Chase The Game", "Winning by 2+ → Shut Up Shop or Hold Shape"), never a vague description instead of a real name. What's NOT confirmed is the exact underlying Mentality/Style/etc. value each named Plan triggers under the hood — so name the Plan, don't invent what it numerically does. Precedence, per the game's own on-page copy: a later sub's Instruction overrides an earlier Plan B, and vice versa — whichever one fires most recently wins and sets the team's approach from that point on.`);
    ...
    if (squad.length) {
      lines.push(`\nMy squad (${squad.length} players) — Name | Pos | Age | Rating | Fitness | Foot | TrueVal (source) | AltPosFit | Ldr/Ment/Exp | FK/Pen/Cor | Spd/Hdg:`);
