...
- **Chat sidebar** (Assistant tab only — the 400px dock keeps the old dropdown+New+Delete header, no room for a full sidebar): a Claude-style collapsible rail on the left, `.assistant-sidebar`/`.assistant-sidebar.expanded` in `style.css` (44px ↔ 240px, `assistantSidebarExpanded` state, persisted to `localStorage` `sf_assistant_sidebar_expanded`, default collapsed). Expanded view lists sessions grouped by `chatSessionGroups()` (Today/Yesterday/Previous 7 days/Older, by `updatedAt`), each row hover-revealing rename (✎, `startRenameSession`/`commitRenameSession`) and delete (🗑) icons. On mobile (`≤768px`) the expanded sidebar becomes a fixed-position drawer with a backdrop that closes it on tap
