...
- **Structured-output reliability regression, caught same day the Phase 9 fixes above shipped**: the owner reported a live reply with an empty Substitutions section and blank/zero set-piece fields ("Penalty: ? (0 Pen)") — visibly worse than before. Live testing (5 real requests, real squad/opponent data) confirmed a real regression: 3/5 came back broken. Root cause was NOT token truncation — `stop_reason` was `end_turn` on every request, output_tokens well under the 8500 cap — it was the model, at `effort:'low'`, sometimes returning schema-*valid* garbage: `subs: []` (an empty array — nothing in `LINEUP_SCHEMA` forbade this) twice, and once genuinely corrupted trailing JSON (garbled text plus a trailing comma) that failed `JSON.parse` outright. The extra Section 2 guidance added earlier the same day (the tightened CB/FB/DM opportunity-cost rule, the relevant-alternatives comparison rule) is the likely trigger — more nuanced tradeoffs to reason through at a fixed low-effort budget made the "harder" fields (the subs array; trailing free-text) more likely to get skipped or garbled, not the reasoning itself being wrong. Two-part fix: (1) **schema hardening** — added `minItems:1` to the `lineup` and `subs` arrays and `minLength:1` to every player-name/slot/set-piece-name string in `LINEUP_SCHEMA`, which structurally forbids the exact empty-array/blank-string patterns observed (Anthropic's structured-output constrained decoding can't sample tokens that would violate the schema). Tried `minimum:1` on the rating number fields too — **rejected by the API outright** ("For 'number' type, property 'minimum' is not supported" — a live 400, caught immediately by testing before it reached users), so a `0` rating relies on the runtime check below instead, not the schema. (2) **Server-side detect-and-retry** — `isLineupReplyBroken(parsed)` in `handleChat` (`cf-worker/index.js`) checks the parsed response for the exact failure shapes seen (`lineup.length<9`, `subs.length<3`, any set-piece name/rating falsy — `!0` catches the zero-rating case even though the schema can't) and, for `lineupMode` only, retries the entire Anthropic call once (`maxAttempts:2`) if the first attempt is broken or fails to parse, before falling back to best-effort. Live-verified 5/5 clean post-fix, including 2 of those 5 that were caught and self-healed by the retry (one JSON-parse failure, one broken-content case) — confirming the retry path itself works, not just that failures happened to stop occurring. Client-side timeout (`_requestAssistantReply` in `assistant.js`) raised 110s→240s for `lineupMode` to cover the worst case of two sequential ~100s+ attempts; non-`lineupMode` messages don't retry server-side and keep their existing 90s. Test harness: same live-request pattern as above, run 5x per change, checking `stop_reason`/`usage` via `wrangler tail -c wrangler.toml` from `cf-worker/` alongside the reply content itself — the `stop_reason` check is what ruled out truncation and pointed at generation-quality instead.
