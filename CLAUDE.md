...
| `GET /api/budgets/public/history/X` | Real per-month transaction ledger — income + expenditure line items, GW-labelled where weekly, ~10 months back |
...
The `/office/*` and `/maintenance/*` endpoints aren't in any official API doc — found by grepping the live app's JS bundle (`https://slowfootball.club/assets/index-*.js`) for `/api/` strings, same technique used for the AI Assistant's mechanics reference. `/facilities/quote` only returns the *next* level's cost from a club's current level, not a full tier table — the full cost/upkeep tables now live in `constants.js` (see below), built by sampling `/facilities/quote` and `/maintenance/*/preview` across ~65 clubs. **The same bundle also embeds the game's actual mechanics** — the `https://slowfootball.club/FAQs` page's Q&A content, and every facility's real per-level formulas (the Office → Facilities page's own render logic) — both as plain data structures in the minified JS, not behind auth. Grep the downloaded bundle for `{q:"` (FAQ entries) or a facility key like `training:{current:` (the per-level formula tables) rather than statistically inferring these from sampled live data — the bundle is authoritative, sampling is a fallback for things (like costs) that only exist server-side.
...
- **Question-type section gating**: `buildDynamicChatContext()` runs two cheap client-side regex classifiers (`_isTransferQuestion()`/`_isTacticsQuestion()`, keyed off `_lastUserMessageText()` — the latest user turn) to decide whether two large, otherwise-always-included blocks are worth their token cost for THIS question: the transfer-targets/recent-deals/transfer-list section (gated on the transfer classifier) and the full opponent-tactics table + 6-section lineup-reply template + tail checklist (gated on the tactics classifier, all three sub-blocks gated together since the template/checklist explicitly reference the table). This is deliberately model-blind retrieval, not LLM-driven — same reasoning as the removed `_isLineupVsOpponentQuestion()`: an LLM that doesn't know a field exists can't know to ask for it, so scoping is a client-side heuristic, never left to the model. Biased toward false positives: when a question matches neither classifier (a general squad/youth question, or anything ambiguous), both sections are included — same as pre-gating behavior — since a missing section produces a wrong/evasive reply while an unneeded one only costs tokens. Squad table, budget, MY_CLUB's last 8 results, the static mechanics reference, and the named-club/player scoped enrichment above are unconditional regardless of category. Live-verified: a pure transfer question shrank `dynamicContext` to ~14k chars (opponent-tactics/template/checklist excluded); a pure tactics question kept it near full size (~51-56k, transfer-targets/deals/listings excluded); an unrelated/ambiguous question included everything, matching pre-gating size.
...
- The frontend POSTs `staticContext`/`dynamicContext` as two separate fields (not one combined `context` string). `handleChat` caps each separately before they reach the model — silent truncation, no error: `staticContext` (the mechanics reference — never needs more than a few KB) at 20,000 chars, `dynamicContext` at 120,000 chars. Re-verify the `dynamicContext` cap if it grows a lot (a full real-scale dynamic context with an opponent named in conversation runs ~55-60k chars currently — the question-type section gating above can shrink this well below that for a question that clearly matches only one category, but never raises it, so the cap is still sized off the full/ambiguous-question case).
...
- **Finance section** (`loadClubFinance()` in `youth.js`, cache `sf_club_finance_v1`, 20min TTL): weekly income/expense breakdown, stadium/matchday (home vs away revenue shown separately — an away fixture's "attendance" is the *host's* total crowd, but the revenue figure is always just your own allocation sold × your ticket price, never conflate the two), sponsorship deals (4 slots: shirt/stadium/kit/**global** — don't forget `global`, easy to drop since 3 of the 4 look like "the" set), season income, and a **Finance History** card backed by `/budgets/public/history/:club` — the real per-month transaction ledger (income + expenditure line items, GW-labelled where weekly) that the game's own FAQ means by "the Office page breaks down the mix, both monthly and across the season." Each card collapses independently, state persisted to `localStorage['sf_finance_collapsed_v1']`.
- **Facility Level Reference**: not a separate table — folded into each facility card as an "All N levels" expandable list (`facExpanded` state, `toggleFacExpand()`). Rebuilt 2026-07-31 **twice**: first from live-data statistical sampling across ~65 clubs (wrong in places — it mistook a scouting-job progress-speed stat for "prospect quality" and concluded Academy level had no effect on it), then corrected by decoding the exact formulas out of the game's own client bundle (`https://slowfootball.club/assets/index-*.js` — functions named `TX`/`$X`/`MX`/`LX`/`AX`/`BS`/`EX`/`WF` in the minified source, plus the game's own FAQ text embedded in the same bundle). The bundle is the actual Office → Facilities page's own logic, so it's authoritative, not inferred. Key corrected facts, all in `constants.js` `FACILITY_LEVEL_FACTS`: **scouted prospect quality depends on three things — Technical Director rating, Scouting level, and Academy level — with Academy the biggest lever** (the game's own copy literally says so); stadium levels go to 8, not 5, with real capacity **and** a `+£3/ticket, +6% shirts, +4% sponsorship per level` bonus most of this rebuild originally missed entirely; training's XP bonus is a `+20%/level` *ceiling* (your realized bonus, shown in the live effects panel, also depends on your coach); academy's real mechanic is an exact big-jump-chance/expected-gain formula (`FACILITY_LEVEL_FACTS.academy`); medical's injury/recovery multipliers are confirmed to level 5; analytics also grants Agent-offer discount/brief-chance tiers and scout missions/gameweek, both previously missing (it only tracked formation unlocks before). `FACILITY_MAX_LEVEL_CONFIRMED` is exact now (from the bundle's own cap function), not statistically inferred — stadium: 8, everything else: 5. Upgrade *costs* still come from the live `/facilities/quote` sampling (confirmed the real Facilities page calls the same endpoint, no separate formula exists) — stadium levels 6-8 were never actually reached by a sampled club, so those specific costs are formula-extrapolated and marked `(proj.)` in the UI (`FACILITY_UPGRADE_COST_PROJECTED`).
- **Upgrade Scenario Planner**: pick a facility + target level, see initial cost (live CEO discount applied, `(proj.)` flagged where extrapolated), weekly upkeep delta (flat-rate facilities only — stadium/scouting/medical/analytics; training/academy scale by headcount in bands, shown via the facility card's expanded level list instead of a single guessed number), and for stadium a revenue/payback projection using the confirmed formula above (capacity × ticket-price-with-bonus, plus the isolated incremental shirt/sponsorship gain — back out the current level's bonus from current revenue before applying the target level's, so the gain isn't double-counted). Ticket revenue itself stays the owner's requested simple model (price × capacity, no occupancy/demand modeling), assuming ~half of fixtures are home games. Scouting/Academy scenarios show the corrected 3-factor quality story above instead of asserting a number the game doesn't expose.
- Research data (per-club facility levels/quotes/staff effects/scouting-job samples, and the extracted FAQ/bundle text) is cached in `exports/*.json` (gitignored) from the 2026-07-31 gathers — re-run a similar sample/bundle-grep if the game changes these numbers.
