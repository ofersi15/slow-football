...
- **Multiple chat sessions**: session switcher (dropdown + New/Delete) in the tab header. Sessions persist to `localStorage` (`sf_chat_sessions_v1`, `{sessions:[{id,title,messages,createdAt,updatedAt}], activeId}`, capped to last 20 sessions × last 30 messages each). Title auto-derives from the first user message. Migrates the old single-thread `sf_chat_history_v1` key into a session on first load
