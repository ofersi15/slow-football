...
- Model: `claude-sonnet-5`, `thinking: {type:'adaptive'}` (must stay on — thinking-disabled causes leaked scratch-work/precision errors on precision-heavy replies), `output_config.effort:'low'`, `max_tokens: 10000` (both modes — free text was raised to match lineup mode after real-matchup testing showed the richer Role/Plan-B content pushing thinking-token variance past the old 6500 cap, truncating replies).
...
- Context is capped at 120,000 chars server-side (`handleChat`) before it reaches the model — silent truncation, no error. Re-verify against this cap if `buildChatContext()` grows a lot (a full real-scale context with an opponent named in conversation runs ~40-44k chars currently, up from ~24-28k before the Player Roles/Plan B/corner-detail additions).
