...
- **`.content{overflow:visible}` at the 768px breakpoint** (needed so tall tab content flows into page-level scroll instead of double-scrolling) means `.content` is a flex item whose automatic min-width defaults to its widest descendant's content width — a bare `<table>` with no wrapper (like Scout's) then drags `.content` itself wide open with no scrollbar anywhere, clipped by `.main`'s row `overflow:hidden`. Fix requires **both** `min-width:0` on `.content` (already added for 768px) **and** a local `overflow-x:auto` wrapper around the table — one without the other doesn't work.
- **Assistant dock on mobile** becomes a `position:fixed` full-screen overlay, but nothing locked the background page's scroll — a `watch` on `assistantDockOpen` in `app.js` now toggles `document.body.style.overflow` while the dock is open on mobile.
- `src/methods/data.js`'s `drawMoneyballChart()` needs its own `import Chart from 'chart.js/auto'` — it doesn't inherit `app.js`'s import (separate module scope), so a missing import here fails silently as "Chart is not defined" in the console with a blank canvas, easy to miss without opening devtools.
- Sibling `v-if` blocks that are meant to be mutually exclusive must chain with `v-else-if`/`v-else`, not repeated `v-if` — a bare `v-if` on the next branch renders independently of the others (see Moneyball's chart-view tabs, which all rendered simultaneously until fixed).
