...
- Context is capped at 120,000 chars server-side (`handleChat`) before it reaches the model — silent truncation, no error. Re-verify against this cap if `buildChatContext()` grows a lot (a full real-scale context with an opponent named in conversation runs ~44-48k chars currently, up from ~24-28k before the Player Roles/Plan B/corner-detail additions — the mentioned-club block now also carries the opponent's real corner-zone assignments and Plan B scenario→Plan pairs, and the squad table carries a Foot column).
